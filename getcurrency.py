...
def output(output_currency,rate):
    print(rate[output_currency][2])

def main():
    parser = argparse.ArgumentParser(description="Get exchange rate from ICBC.")
    parser.add_argument("--sell", type=str, required=False)
    args = parser.parse_args()
    rate = get_rate()
    output(args.currency,rate)
